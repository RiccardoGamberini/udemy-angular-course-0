...
export class CustomDivComponent {
  ...
  * - The value of aParagraph variable (.....aParagraph: ElementRef.....) is the first HTML element that match the selector passed to @ContentChild.
  * - With ContentChild custom-div component can access to it's own content. It's own content is:
  *   .....<p #aParagraph *ngFor="let item of anArray">Id = {{item.id}} | data = {{item.data}}</p>.....
  ...
  * */
  ...
  aFunction() {
    ...
    this.message = `The aParagraph ElementRef object is printed in console. It's value is: ${this.aParagraph.nativeElement.textContent}`;
  }
}
